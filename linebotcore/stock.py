...
class StockHandler:
    def __init__(self, text: str, user_id: str = None, key: str|None = None, code_type: str = 'cryptocurrency'):
    ...
    def detect(self):
        ...
        for currency in result:
            ...
            if data['valid']:
                if self.key == '關注':
                    if StockWatchList.objects.filter(user_id=self.user_id, code=currency, code_type=self.code_type).exists():
                        self.messages.append(
                            TextSendMessage(text='{} ({}) 已經存在你的關注清單內了!'.format(currency, self.code_type))
                        )
                    else:
                        StockWatchList.objects.create(user_id=self.user_id, code=currency, code_type=self.code_type)
                        self.messages.append(
                            TextSendMessage(text='已新增至專注清單: {} ({})'.format(currency, self.code_type))
                        )
                elif self.key == '取消':
                    if StockWatchList.objects.filter(user_id=self.user_id, code=currency, code_type=self.code_type).exists():
                        StockWatchList.objects.filter(user_id=self.user_id, code=currency, code_type=self.code_type).delete()
                        self.messages.append(
                            TextSendMessage(text='成功從關注清單移除 {} ({}) !'.format(currency, self.code_type))
                        )
                    else:
                        self.messages.append(
                            TextSendMessage(text='{} ({}) 不再你的關注清單內!'.format(currency, self.code_type))
                        )
                else:
